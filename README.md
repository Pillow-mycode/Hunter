...
在Hunter/hunter-server目录下创建 `.env` 文件（如果目录下右键创建文件后发现文件没有显示，则需要勾选显示隐藏文件选项，在窗口的上方选项栏点击视图，选择显示隐藏文件）并进入，粘贴并配置以下内容（注意：这里有四部分，可以仅配置第一个默认部分其他为空即可，如需要单独为不不同智能体选择不同模型直接填入信息即可，默认优先级：专门配置 > 默认配置）：
